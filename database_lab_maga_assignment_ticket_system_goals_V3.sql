...
-- 获取用户表中的id
DELIMITER ||
drop procedure if exists proc_get_user_id ||
create procedure proc_get_user_id(
    in  get_user_name  varchar(20),
    out found_user_id    int,
    out id_is_found   boolean
    )
    BEGIN
        IF(check_user_name(get_user_name) != 1) THEN set id_is_found = false;
        ELSE
            select user_id from user where user_name = get_user_name into found_user_id;
            set id_is_found = true;
        END IF;
    END ||
DELIMITER ;
...
set @isFD = false;
set @tmp_id = 2;
call proc_get_user_id("DFA", @tmp_id, @isFD);
...
create procedure proc_delete_user(
    ...
    in  delete_user_pwd  varchar(16),
    out is_delete        boolean   )
    BEGIN
        DECLARE id INT default 0;
        DECLARE isF boolean default false;
        call proc_get_user_id(delete_user_name, id, isF);
        IF(isF = true) THEN delete from user where user_id = id and password = delete_user_pwd;
            IF(check_user_name(delete_user_name) = 0) THEN set is_delete = true;
            END IF;
        END IF;
    END ||
DELIMITER ;
